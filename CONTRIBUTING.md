...
Strands is an agent SDK. We implement the agent runtime once in TypeScript, compile it to a WebAssembly component, and host it from every language through a thin native wrapper. This guide covers how to set up the repo, how the architecture works, and how to validate changes.
...
graph TD
    ...
    RS -->|PyO3| PY_EXT["_strands.so/.dylib"]
    RS -->|UniFFI| KT_LIB["libstrands.dylib"]

    PY_EXT -->|maturin develop| PY["strands-py (Python package)"]
    PY_EXT -->|strands-stubs| PY_STUBS["strands-py/**/*.pyi"]
    PY_STUBS --> PY

    KT_LIB -->|uniffi-bindgen| KT_SRC["strands.kt (generated bindings)"]
...
| `strands-rs/`     | Rust       | WASM host: Wasmtime, AOT compilation, WASI HTTP, PyO3/UniFFI        |
...
| Python bindings (Rust/PyO3 code)      | `npm run dev -- validate py-bindings` |
